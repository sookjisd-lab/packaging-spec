...
| **Excel 생성** | xlsx (SheetJS) | 0.18.5 | Excel 파일 생성 |
...
│
├── types/
│   └── index.ts                # 모든 TypeScript 타입/인터페이스 정의
│
├── store/
│   └── formStore.ts            # Zustand 스토어 (전역 상태 관리)
│
├── utils/
│   └── exportUtils.ts          # PDF/Excel/JSON 내보내기 유틸리티
│
└── components/
...
Excel 다운로드:
  getFullData() → xlsx.js → 다중 시트 생성 → 다운로드
...
### 7.1 착인 구성 (MarkingComposition) 조건부 필드
...
### 8.1 Tailwind CSS 커스텀 클래스 (index.css)
...
| 파일 | 라인 수 | 역할 |
|------|---------|------|
| types/index.ts | ~370 | 타입 정의 |
| store/formStore.ts | ~470 | 상태 관리 |
| utils/exportUtils.ts | ~220 | 내보내기 |
| App.tsx | ~110 | 메인 앱 |
| **컴포넌트 총합** | ~2,000 | UI 렌더링 |
| **전체** | ~3,200 | - |
...
└───────┬───────┘   └───────┬───────┘   └───────┬───────┘
        ...
        └───────────────────┼───────────────────┘
            ...
            ▼               ▼               ▼
     ┌──────────┐    ┌──────────┐    ┌──────────┐
     │  types/  │    │ persist  │    │ export   │
     │ index.ts │    │(localStorage)│ │ Utils   │
     └──────────┘    └──────────┘    └──────────┘
```
